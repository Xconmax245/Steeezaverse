  ...
  return (
    <ClickBlackHole dotSize={3} count={65} coreRadius={12} color="#ffffff" className="w-full">
      ...
      >
        ...
        {isMobile ? (
          <motion.div
            className="absolute left-1/2 -translate-x-1/2 z-10 select-none pointer-events-none flex justify-center w-full"
            style={{ bottom: "16vh" }}
            initial={{ opacity: 0, y: 15 }}
            animate={playAnimations ? { opacity: 1, y: 0 } : { opacity: 0, y: 15 }}
            transition={{ duration: 1.0, delay: 0.7, ease }}
          >
            <span ref={centerRef} style={{ ...wordStyle, fontSize: "clamp(30px, 12vw, 60px)" }}>
              STEEZAVERSE
            </span>
          </motion.div>
        ) : (
